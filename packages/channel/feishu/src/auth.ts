 ...
 * absent key, so an optional field is spread in or not at all. Taking the value
 * as an argument means it is computed once, which is the other half of why the
 * call sites used to need a non-null assertion.
 * @param key - the field name.
 * @param value - its value, or undefined to omit the field.
 * @returns an object with the one field, or an empty one.
 */
...
function userIdentityOf(value: unknown): AuthUserIdentity | undefined {
  ...
  return {
    ...base,
    ...optional('openId', text(source.openId)),
    ...optional('userName', text(source.userName)),
    ...optional('tokenStatus', text(source.tokenStatus)),
    scopes: scope === undefined ? [] : scope.split(/\s+/).filter(Boolean),
    ...optional('expiresAt', text(source.expiresAt)),
    ...optional('refreshExpiresAt', text(source.refreshExpiresAt)),
  }
}
...
export class LarkAuth {
  ...
  async status(requestedDir: string): Promise<AuthStatus> {
    ...
    return {
      ...
      installed: true,
      configured: true,
      ...optional('appId', text(json.appId)),
      ...optional('brand', text(json.brand)),
      ...optional('identity', text(json.identity)),
      ...bot === undefined ? {} : { bot },
      ...user === undefined ? {} : { user },
    }
  }
  ...
  private watchBind(
    ...
    child.once('exit', (code: number | null) => {
      ...
      void this.status(pending.configDir).then((status) => {
        pending.outcome =  status.configured
          ? { granted: true }
