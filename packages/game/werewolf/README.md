...
The deterministic Werewolf game runtime: `ctx.werewolf` definition registries, rule-set compilation, durable `werewolf/*` events, reducer, phase engine, bot continuity, authorized projections, fresh one-shot decision runner, thin `ctx.games` adapter, and typed Host methods. The engine remains the only authority for role assignment, legal actions, effects, phases, and victory. The dedicated Web game view is provided by [`dsh-client-ui-werewolf`](../../client/ui-werewolf).
