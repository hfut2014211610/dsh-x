...
function parseHeaderRecord(record: Buffer): {
  readonly meta: SessionHeader
  readonly restore: SessionFormatRestore
  /** Raw parsed header, retained so an overlap can rebuild the restore. */
  ...
  return { meta: fromHeaderLine(parsed).meta, restore, headerValue: parsed }
}
...
export class SessionLogScanner {
  ...
  private restore: SessionFormatRestore
  /** Raw parsed header for rebuilding the restore when an overlap displaces fed rows. */
  private readonly headerValue: unknown
  /** Header byte length: the committed offset when no row survives. */
  private readonly headerBytes: number
  /** Rows fed to the restore with their end offsets, replayable after a displacement. */
  private fedRows: Array<{ readonly row: unknown; readonly endByte: number }> = []
  ...
  constructor(
    ...
    this.headerValue = parsed.headerValue
    this.headerBytes = headerRecord.length
  ...
  private consumeEventLine(line: Buffer, endByte: number): void {
    ...
    // A row that restarts BELOW the rows already fed overlaps them rather
    ...
    // a handful of duplicated seqs. SessionFormatRestore is single-pass with
    // no rewind, so displacement rebuilds it over the retained prefix and
    // replays the rows the overlap spared.
    const rowSeq = typeof decoded === 'object' && decoded !== null && !Array.isArray(decoded)
      && typeof (decoded as { seq?: unknown }).seq === 'number'
      ? (decoded as { seq: number }).seq
      : undefined
    if (rowSeq !== undefined && rowSeq < this.eventCount) this.displaceOverlap(rowSeq)
    try {
    ...
    this.fedRows.push({ row: decoded, endByte })
    this.eventCount += 1
    this.committedBytes = endByte
  }

  /**
   * Drop the fed tail an overlapping row numbers twice and rebuild the restore
   * over the spared prefix. Only rows fed while no issue stood are retained,
   * so the replay decodes clean; the overlapping row itself feeds next through
   * the ordinary path above.
   * @param rowSeq - the overlapping row's starting seq, below the fed count.
   */
  private displaceOverlap(rowSeq: number): void {
    this.overlaps += this.eventCount - rowSeq
    this.overlapFloor = Math.min(this.overlapFloor ?? rowSeq, rowSeq)
    const retained = this.fedRows.slice(0, rowSeq)
    this.restore = sessionFormatCatalog.createRestore(this.headerValue, {
      recovery: 'strict',
      validation: 'transformed',
    })
    this.fedRows = []
    this.eventCount = 0
    this.committedBytes = this.headerBytes
    for (const fed of retained) {
      this.restore.decodeRow(fed.row)
      this.fedRows.push(fed)
      this.eventCount += 1
      this.committedBytes = fed.endByte
    }
  }
}
