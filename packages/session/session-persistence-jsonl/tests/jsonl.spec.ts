...
describe('JsonlSessionPersistence: scanLog unit', () => {
  ...
  it('lets a later append that restarts below the events read displace them', () => {
    const log = [
      JSON.stringify({ type: 'session', version: SESSION_FORMAT_VERSION, id: 'o', createdAt: 1, isSeeded: false, delegationDepth: 0 }),
      JSON.stringify({ type: 'turn/start', seq: 0, time: 1, data: { turn: 1 } }),
  ...
  it('reports no overlap for a log one writer wrote', () => {
    expect(scanLog(Buffer.from(
      [JSON.stringify({ type: 'session', version: SESSION_FORMAT_VERSION, id: 'o2', createdAt: 1, isSeeded: false, delegationDepth: 0 }),
