...
| [`usage-stats/`](usage-stats/README.md) | Serves per-request model token usage (`usageStats` unit) plus the `/usage` report command | registers on `ctx.sessionProjections`, `ctx.commands` |
