---
description: "Model-facing document tools: document_search/read/outline/create/edit with version-carrying projections."
...
This package gives the model five document tools plus the writing guidance prompt section. Result projections carry more than bodies: reads prefix `path`, `version`, and clip-only `truncated` so guarded edits can name `base_version` and line locators stay aimed at whole-document lines; creates and edits report the version their write produced so follow-ups skip a re-read; call views declare `locations` diff cards the deliverables UI reads by render intent. Use it when the model must find, read, and revise documents through tools.

## Table of Contents

- [Result projections](#result-projections)
...
What reaches the model is `render`, never the validated `output.schema` value: keep `path`/`version`/`truncated` on reads and the produced version on writes, or guarded edits fail as stale and clipped locators misfire. Deliverables recognize mutations by render intent (`locations`), never by tool name.
