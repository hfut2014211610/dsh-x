---
description: "Single-runtime guard: refuse to boot a second dsh runtime against a harness home that already has one."
...
This package refuses to boot a second `dsh` runtime against an occupied harness home, because session logs number events with no lock and two writers interleave numbering into unreadable logs. The guard is deliberately blunt: refusing the situation instead of managing cross-process ownership. Refusal goes through `ctx.appExit`, never a thrown `apply`, and the pid-pinned claim file is taken over — not obeyed — when its pid is dead. One-shot commands that never boot this row are unaffected. Use it in every runtime composition; turn `enforce` off only for deployments that genuinely accept shared-log damage.

## Table of Contents

- [The guard is deliberately the blunt one](#the-guard-is-deliberately-the-blunt-one)
- [Refusing means `ctx.appExit`](#refusing-means-ctx-appexit)
- [The claim is a file, honoured only while its pid is alive](#the-claim-is-a-file-honoured-only-while-its-pid-is-alive)
...
`src/claim.ts` decides everything as a pure function over a parsed claim, this process's pid, and a liveness predicate — the caller owns the filesystem and the process table, which is what lets every branch test without either. A pid alone is not an identity: never obey a dead claim, and never refuse over a crash note.
