...
| [`game/`](game/README.md) | 确定性狼人杀游戏核心与经典定义 | 产品：稳定 API |
...
| [`web/`](web/README.md) | Web 能力系列：seam、搜索／获取提供方、面向模型的工具 | 产品：稳定 API |
| [`writing/`](writing/README.md) | 写作模式：文档能力系列、工具和策略提示词 | 产品：稳定 API |
...
| [`session/`](session/README.md) | 持久会话数据平面：持久化 seam + JSONL/SQLite 后端、投影、基于日志的标题、上报 | 产品：稳定 API |
| [`session-query/`](session-query/README.md) | 会话检索系列：逻辑语料库、有界读取、血缘、语义过滤、SQLite 全文搜索 | 产品：稳定 API |
