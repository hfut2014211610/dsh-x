 ...
 *
 * First-request conditions established by the upstream reproduction runs
 ...
 *     (a prebuilt profile package overwrote it with `adapterDefaults.maxTokens`
 *     in one observed build). `bootstrapMaxTokens` is therefore OPT-IN: leave
 *     it unset to run the Minimal schema at the adapter default, or set it to
 *     cap the first request. When set, the cap is stripped after promotion —
 *     the next request's seed proposal carries the previous header's
 *     maxTokens forward, so the release must be explicit.
 *
 *  3. Injected context is NOT this plugin's concern: the companion
 *     `context-gate` plugin (mounted as the FIRST row) owns the unified
 *     injection control — runtime-context suppression on the assembly path
 *     and a claimed-baseline deny on the pre-step waterfall, both keyed to
 *     the same epoch-aware promotion phase.
 *
 ...
 *
 * POST-PROMOTION RESIDENT SET (user-measured upstream): the promoted phase
 * does NOT dump the whole Standard catalog at once — that dump pulls the
 * trajectory back to standard-like behavior (the root cause of the
 * post-promotion regression measured on the zero-tool variant). Instead the
 ...
 * pair — the persistent `bash` shell and `str_replace_editor`. The upstream
 * reproduction measured this schema anchoring 5/5 at the adapter-default
 * maxTokens while every standard-family schema failed 11/11.
 */
...
export function apply(ctx, config) {
  ...
  const unlockedFor = (session) => {
    ...
    if (session === undefined || typeof session.snapshotEvents !== 'function') return unlocked
  ...
  if (bootstrapMaxTokens !== undefined) {
    // Same registration discipline as the pre-step strip in context-gate.mjs:
    // `prepend` keeps this listener the OUTERMOST transform of the
    // agent/request waterfall for the same registration-order reasons (loader
    // row application is concurrent; row order alone does not decide listener
    // order), so a later listener can never override the first-round budget
    // after we set it.
