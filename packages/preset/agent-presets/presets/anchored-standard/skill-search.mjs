 ...
 * trajectory (measured by the upstream reproduction: 0/9 sessions anchored
 * with the catalog present vs ~81% without). We remove the catalog injection
 * entirely and expose two small tools instead — the Claude tool-search
 * pattern:
 *
 ...
 * dsh-tool-skill, and keeps its visibility rule: only skills whose
 * `invocation.modelInvocable` is true reach the model. If skills are
 * unavailable the tools answer with a short message instead of throwing.
 *
...
/** Whether a skill may be advertised to and loaded by the model (dsh-tool-skill's visibility rule). */
const modelInvocable = (skill) => skill?.invocation?.modelInvocable === true
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        const matches = all.filter(modelInvocable).filter((skill) => {
