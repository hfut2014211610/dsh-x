...
- **交互状态** —— 大厅、遮盖揭示（两步交互，不依赖翻牌动画）、每个座位使用一个中性徽记并带粘性阶段标题的 Signal Circle 游戏桌、带隐私提示的夜间聚焦、显示当前发言人/进度与按序发言或明确过麦记录的白天轮流发言、带单选组选择与粘性确认的投票、观战标注，以及带复盘检查点的结算。全部计划中的存活座位完成发言或过麦后才会出现投票。狼人杀 preset 会话在提问与审批接管之后选择一个空的 `conversation.composer` 替代项，使游戏动作表单成为唯一的常规输入，同时保留系统等待的回答入口。
