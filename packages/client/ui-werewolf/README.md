...
- **View registration** — injects the `conversation.view` slot entry `werewolf` (order 6) with a bilingual label, and declares it the preferred view for sessions whose `agentPreset` is `werewolf` without overwriting the user's persisted tab.
...
- **Invalidation refresh** — subscribes to the forwarded `game/projection-invalidated` host event before the first read, ignores events for other games, and re-reads through `getView`; mutation responses replace the projection directly, so no client-side game state exists beyond presentation preferences.
...
- **Generic action form** — renders the closed spec vocabulary only: `player-target`, `choice`, `text`, and `compound`; field ids stay stable, text drafts are bounded by `maxChars`, and pass/skip appears only when `allowSkip` permits it. No HTML, CSS, callbacks, or component names can arrive from configuration.
- **Accessibility** — seats announce state through text, keyboard traversal reaches every control in DOM order, radio groups use `role="radio"` with `aria-checked`, busy and error regions use `role="status"`/`role="alert"`, and the phase heading takes `tabindex={-1}` for focus restoration after committed transitions.
