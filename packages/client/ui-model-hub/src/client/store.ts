...
export interface ModelHubState {
  ...
  routeByModel: Partial<Record<string, string>>
  ...
  credentials: Partial<Record<string, CredentialState>>
  /** Vendor presets for the editors' dropdowns (from the pi-ai catalog). */
  presets: ProviderPreset[]
  /** Latest probe outcome per model id. */
  probeResults: Partial<Record<string, ProbeResult[]>>
  /** Probe in flight per model id. */
  probing: Partial<Record<string, boolean>>
}

/**
 * Convert a rejected wire value to user-visible text.
 * @param error - caught rejection value.
 * @returns an Error message or the value's string representation.
 */
...
/**
 * Collect provider ids still referenced by at least one model.
 * @param models - complete authored model table.
 * @returns the referenced primary-provider ids.
 */
...
export class ModelHubStore {
  ...
  async probe(id: string): Promise<void> {
    this.store.update((s) => {
      s.probing[id] = true
      s.probeResults[id] = undefined
    })
    const settle = (results: ProbeResult[]): void => {
      this.store.update((s) => {
        s.probing[id] = undefined
