...
export function ModelEditor({
  open, editing, existing, models, providers, providerPresets, presets, t, onSave, onClose,
}: ModelEditorProps) {
  const [id, setId] = useState(editing ?? '')
  const [provider, setProvider] = useState(existing?.provider ?? providers.at(0) ?? '')
  ...
  const addFallback = (): void => {
    const spare = providers.find(key => key !== provider) ?? providers.at(0)
    ...
    >
      <div className={styles.form}>
        ...
        <IdentityFields
          ...
          identityLabel={t('modelId')}
          identityValue={id}
          identityPlaceholder="my-model"
          displayNameLabel={`${t('displayName')}（${t('optional')}）`}
          displayName={name}
          readOnly={editing !== null}
          onIdentityChange={(value) => { setIdTouched(true); setId(value) }}
              ...
              <div className={styles.checks}>
                ...
                <label className={styles.check}>
                  <input
                    type="checkbox"
                    checked={image}
                    onChange={(event) => { setImage(event.currentTarget.checked) }}
                  />
                  <span>image</span>
                </label>
              </div>
