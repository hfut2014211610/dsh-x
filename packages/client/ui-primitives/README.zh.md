...
纯 React 原子组件（零 cordis）：StateDot、DisclosureRow、ic_ds_* 图标、Button/Pill/Menu/Modal/Input、Toast 短时横幅、OnboardingSurface 首次使用接管层（portal 到 body 的遮罩加不透明展示层，在且仅在自身生命周期内保持 `#root` 为 `inert`）、markdown 家族（MessageText/MarkdownText/JsonBlock）、只读 JsonTree 检查器、`useAnchoredMaxHeight` 钩子（把底部锚定的浮层高度收敛到锚点上方的视口空间，并在 resize、scroll 与调用方提供的依赖变化时重新测量）、`useAnchoredPosition` 钩子（让固定定位的浮动面板跟住锚点：测量、偏移、按视口边距钳制，并在捕获阶段滚动、窗口缩放与面板自身尺寸变化时重新定位）、TerminalBlock、DiffBlock、ReadBlock、SearchBlock、WebBlock，以及 ResizeHandle 分隔条——两栏面板把它放在两栏之间，让其中一栏可调宽（指针捕获、移动合并到一帧、支持方向键、`role="separator"` 并实时更新 `aria-valuenow`；`side` 指明报出的宽度属于哪一侧的列，因此调右侧列时同一个手势方向相反；应用外框保留它自己那个分隔条，那个是覆盖层定位、且与外框的列求解耦合）。
