...
`/client` 导出表层包含插件主体（`apply`／`inject`）、`LayoutController` 和五个 owner-share 接口，其中包括传给 `shell.surface` 的 Host 确认 preset。AppFrame、面板 store 与让步求解器仍属于包内部。
