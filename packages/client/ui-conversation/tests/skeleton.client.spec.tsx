...
function mount(
  ...
  // A pending takeover interaction rides the unified Session status map; seed
  // it when a case asks for one.
  const statusSeed: SessionStatusSnapshot = options.pendingTakeover === true
    ? new Map([[SID, {
      running: false,
  ...
  const useSessionStatus = bindSnapshotSelector(
    createSnapshotStore<SessionStatusSnapshot>(statusSeed),
  )
  ...
  const useConversationViews = bindSnapshotSelector(createSnapshotStore(viewTabs))
  ...
  const chat = store
  return {
    view, chat, store, wiring, sink, retargetWorkspace, session, conversation, sessions, slotCalls, lineageOwners, seatOwners, open,
