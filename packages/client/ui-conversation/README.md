...
Feature plugins can call `ctx.conversation.declareCompanionView` to place one registered view beside a specific active view. The resolver supplies the companion id and localized panel label. The session body renders both entries through the same owned `conversation.view` render share, hides the mutually exclusive tab strip, gives the companion its own scrollport, and positions the resident composer in that column. A separator between the two makes the companion column adjustable: it starts at whatever the shipped CSS width resolves to for the viewport, measured once the panel is laid out, and a percentage ceiling keeps a column dragged wide from swallowing the primary view when the window later narrows. A missing, self-referential, or unregistered companion is ignored, and sessions without a companion retain the ordinary ring.

`ctx.conversation.declarePreferredView(viewId, matches)` declares a session-owned view rather than another switchable tab. The view is omitted from the tab strip and can become active only when its matcher accepts the current session; a match takes precedence over the persisted ordinary tab without writing it, and a later mismatch restores that tab. A matched registered view also promotes a blank session out of the Hero immediately. The root, header, and body subscribe to the session-list projection of the preference so a confirmed agent-preset switch updates the assembled layout in the same render cycle. The composer selector receives that same Host-confirmed `agentPreset`, allowing a dedicated product view to replace ordinary message input without guessing from a client route or title.

Chat business rows are independent registry contributions rather than a closed built-in union. A client plugin declaration-merges its typed `ChatNodeDataMap` key, registers a `ConversationNodeDefinition` on `ctx.conversationEvents`, and registers the matching keyed renderer on `conversation.chat.node`; it does not modify Session folds or a central renderer switch. The [Conversation Node reference](../../../docs/subsystems/conversation.md) covers stable event ids, append/prepend replay, Location data, and renderer constraints.
...
interface ComposerChainProps {
  /** Composer chain currency: interactions for feature-owned takeover selectors. */
  interactions?: readonly SessionPendingInteraction[]
  ...
  session?: SessionSnapshot | ConversationSnapshot | undefined
