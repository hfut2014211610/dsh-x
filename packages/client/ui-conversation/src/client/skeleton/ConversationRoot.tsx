...
export function ConversationRoot({
  ...
  // `views` is the HEAD-era preferred/companion registry; upstream callers may
  // omit it, in which case no session owns a preferred view. The subscription
  // below stays unconditional via stable fallbacks to keep hook order fixed.
  ...
  // HEAD-era `composerPhase` was the blank/engaging discriminator; upstream
  // renamed the same fact `shellPhase` without changing its blank meaning.
  const composerPhase = shellPhase
  ...
  const settling = sessionId !== undefined && (
    (!preferredSessionView
      && composerPhase === 'blank' && openState === 'loading'
      && summaryBlank !== true)
