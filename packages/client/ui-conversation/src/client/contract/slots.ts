...
import type { ViewCompanion, ViewTab } from './views.ts'
...
/** Business callbacks injected into the conversation slot, the resident Conversation shell. */
...
export interface ConversationSessionInjected {
  ...
  /** Bind the input draft persistence mirror to the Session-owned store instance. */
...
export interface ConversationSessionHeaderInjected {
  ...
  readonly hooks?: { readonly conversationViews: ObservableSnapshot<readonly ViewTab[]> }
  /** Select a real Session through the runtime navigation owner; select a Session through the Session Controller. */
