...
/**
 * The conversation-adjacent surfaces: the new-session chip naming the
 * next session's preset, and the session header's blank-session picker or
 * read-only running label. The split is the host's rule — a session's
 * history is produced under its preset's tools, so the choice is only ever
 * offered before one starts.
 */
...
function renderLabel(
  summary: { blank: boolean; agentPreset?: string; projectionValues?: { agentPreset?: string | null } } | undefined,
  ...
  const seatStore = createSnapshotStore<AgentPresetSeatState>({
    ...SEAT_READY,
    current: (summary?.projectionValues?.agentPreset as string | undefined)
      ?? summary?.agentPreset ?? SEAT_READY.current,
  })
  ...
  const view = render(<AgentPresetLabel {...({
    ...
    useSessionRetainInfo: (selector: (value: SessionRetainInfo | undefined) => unknown) =>
      selector({ referenceCount: 1, retainedBy: { mainView: 1 } }),
...
describe('the session-header label', () => {
  ...
  it('falls back to the id, and to the generic hint, when metadata is absent', () => {
    renderLabel({ blank: false, agentPreset: 'mine', projectionValues: { agentPreset: 'mine' } })
  ...
  it('keeps the preset picker available while a preferred view owns a blank session', async () => {
    const { loadSeat, select } = renderLabel({ blank: true, agentPreset: 'standard', projectionValues: { agentPreset: 'standard' } })
