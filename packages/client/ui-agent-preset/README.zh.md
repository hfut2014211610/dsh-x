...
第三个表层位于会话标题旁。会话仍为空白时，它复用新建会话选择器，因此 Writing 等优先工作区替换 Hero 后，离开该 preset 所需的控件仍然可用。第一个轮次开始后，它会变为静态装饰，仅显示**本会话**所运行的 preset；此时再提供控件，等于承诺一次宿主会断然拒绝的切换。它从会话自身的摘要读取 preset，并在 General 行所读的同一份名单上解析显示名称。转发的 owner 事件 `agent-preset/selected` 会在每个标签页中把已经提交的空会话切换折进这份共享摘要；发起方标签页可能已经采用 RPC 回执，而合并是幂等的。
...
preset 文件提供一套未国际化的 `name` 与 `description`，Web 将其用于所有 `user` 行和未知的 `system` 行。对于随附的各个 id（`standard`、`anchored-standard`、`code`、`minimal`、`cordis`、`writing` 与 `ued`），只有名单将该行标记为 `system` 时，Web 才会从当前 locale 解析这两个字段；同名的 `user` preset 仍使用其文件元数据。
