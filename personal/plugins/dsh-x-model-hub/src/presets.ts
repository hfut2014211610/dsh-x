 ...
 *
 * @module @personal/dsh-x-model-hub/presets
 */

import { catalogModels } from '@deepseek-ai/dsh-llm-pi-ai/src/catalog.ts'
import type { HubModel } from './compile.ts'
...
function deriveVendor(spec: VendorSpec): ProviderPreset | undefined {
  ...
  return {
    key: spec.key,
    label: spec.label,
    baseURL: spec.baseURLOverride ?? picked[0]!.baseUrl ?? '',
...
/** The preset a provider was created from, when it carries the `preset` marker. */
