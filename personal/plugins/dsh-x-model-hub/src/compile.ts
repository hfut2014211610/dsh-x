 ...
 *
 * @module @personal/dsh-x-model-hub/compile
 */
...
const hubProvider = z.object({
  ...
  endpoints: z.dict(z.string(), z.union([...supportedProtocols()])) as unknown as z<Record<string, string>>,
...
function groupModels(config: Pick<Config, 'providers' | 'models'>): { groups: ModelGroup[]; protocolCounts: Map<string, number> } {
  ...
  for (const [id, model] of Object.entries(config.models ?? {})) {
    ...
    if (model.reasoningEfforts !== undefined) entry.reasoningEfforts = model.reasoningEfforts as NonNullable<PiAiModelProfile['reasoningEfforts']>
...
function routeNameOf(group: ModelGroup, protocolCounts: Map<string, number>): string {
  return protocolCounts.get(group.providerKey)! > 1 ? `${group.providerKey}${ROUTE_SEPARATOR}${group.api}` : group.providerKey
}
...
export function compileRoutes(config: Pick<Config, 'providers' | 'models'>): Record<string, PiAiProviderProfile> {
  ...
  for (const group of groups) {
    ...
    const multi = protocolCounts.get(group.providerKey)! > 1
...
export function compileChains(config: Pick<Config, 'providers' | 'models'>): Record<string, string[]> {
  ...
  for (const [id, model] of Object.entries(config.models ?? {})) {
    ...
    chains[id] = placements.map(placement => nameByPair.get(`${placement.provider}\0${placement.api}`)!)
  }
  return chains
}
