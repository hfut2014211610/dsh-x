 ...
 *
 * @module @personal/dsh-x-model-hub
 */
...
import type { HubProvider } from './compile.ts'
...
export type { Config as HubConfig, HubModel, HubModelFallback, HubProvider, PiAiCompatProfile } from './compile.ts'
...
export type { ImportContext, ImportNote, ImportPlan, ImportSkipReason } from './decompile.ts'
...
/**
 * The document the wire gateway serves to the settings page: the resolved
 * authoring section plus the seam's writability and the linkage facts the page
 * renders — which route each model compiles to, whether the last reconcile
 * landed, the credential state of every provider, and the current default
 * model selection.
 */
export interface HubDocView {
  ...
  /** Ordered fallback chain per multi-provider model id (route names, primary first). */
  chains: Record<string, string[]>
  /** Last failed reconcile's message, while generated routes lag the authoring section. */
  reconcileError?: string
  /** The `agent-default-model` selection, when that namespace is registered. */
  defaultModel?: { provider: string; model: string; reasoningEffort?: string }
  /** Credential state per provider that names a reference: resolvable or not, ref well-formed or not. */
  credentials: Record<string, { configured: boolean; valid: boolean }>
}

/** The result of importing hand-written `llm-pi-ai` routes into the hub. */
export interface ImportOutcome {
  /** Provider keys added to the authoring section. */
  providers: string[]
  /** Model ids added to the authoring section. */
  models: string[]
  /** One entry per skipped route or model, with a machine-readable reason. */
  notes: ImportNote[]
}
...
export class ModelHubGateway extends TypertRemoteService {
  ...
  async getDoc(): Promise<HubDocView> {
    ...
    return {
      providers: (doc.providers ?? {}) as Record<string, unknown>,
      models: (doc.models ?? {}) as Record<string, unknown>,
   ...
   */
  @Remote('saveProvider')
  async saveProvider(key: string, value: unknown, apiKey?: unknown): Promise<{ ok: true }> {
  ...
  /** The vendor preset table for the page's dropdowns, derived from the pi-ai catalog. */
  @Remote('listPresets')
  ...
  async probeModel(id: string, timeoutMs?: number): Promise<{ results: ProbeResult[] }> {
    ...
    return { results: plan.map(entry => entry.stale ?? byRoute.get(entry.route)!) }
  }
  ...
  @Remote('saveModel')
  async saveModel(id: string, value: unknown): Promise<{ ok: true }> {
