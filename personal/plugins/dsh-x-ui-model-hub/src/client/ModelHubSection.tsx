...
export function ModelHubSection(props: ModelHubSectionProps) {
  ...
  const ready = controller !== undefined && useSnapshot !== undefined && t !== undefined
  const state = ready ? useSnapshot(snapshot => snapshot) : undefined
  useEffect(() => {
    if (controller !== undefined) void controller.load()
  }, [controller])
  if (!ready || state === undefined || t === undefined || controller === undefined) return null
    ...
    <div className={styles.root}>
      ...
      <div className={styles.sectionHead}>
        ...
        <div className={styles.headActions}>
          ...
          <Button icon={<IconPlusOutline16 />} onClick={() => setEditor({ kind: 'provider', key: null })}>{t('addProvider')}</Button>
        </div>
      </div>
      ...
      <div className={styles.sectionHead}>
        ...
        <Button icon={<IconPlusOutline16 />} onClick={() => setEditor({ kind: 'model', id: null })}>{t('addModel')}</Button>
      </div>
            ...
            return (
              <li key={id} className={styles.row}>
                <div className={styles.rowMain}>
                  ...
                  {state.probing[id] === true ? <span className={styles.rowMeta}>{t('probing')}</span> : null}
                ...
                <div className={styles.rowActions}>
                  <Button variant="outline" disabled={state.probing[id] === true} onClick={() => void controller.probe(id)}>
                    {t('probe')}
                  </Button>
                  {isDefault ? null : (
