...
## L1 长任务结果（2026-08-19）：轨迹差异在，产出差异不在

判分脚本先对当时的 HEAD 标定过（干净树 PASS / 种入后 FAIL / 还原后 PASS），6 个会话全部有效，锚定臂契约 3/3 干净。

任务：三处互不相关的跨包回归（`packages/core/session` 的序号连续性、`system-prompt` 的段排序、`tools` 的代理对截断），共 15 个失败测试。判分是三个套件各自 exit 0，且未改任何测试文件。

### 产出

| 条件 | 判分 | 步数 |
|---|---|---|
| anchored | **PASS / PASS / PASS** | 26 / 29 / 51 |
| wide | **PASS / PASS / PASS** | 41 / 35 / 47 |

**两条臂都是 3/3 全过，没有产出差异。** 步数均值 35.3 对 41.0，但区间大幅重叠（26–51 对 35–47），n=3 下说明不了什么。

### 轨迹

同一批会话的指纹仍然是完全分开的：

| 组 | 推理块 | we-only% | letMe% | 块中位长度 |
|---|---:|---:|---:|---:|
| anchored | 62 | **71.8%** | 3.9% | 488 |
| wide | 38 | 0.0% | **95.2%** | 2596 |

**这是本仓库第一次同时拿到两种测量：轨迹形态差异大且稳定，产出差异测不到。**

### 这个结果能说什么，不能说什么

**能说的**：在一个两种条件都做得下来的任务上，锚定条件没有把活干砸。之前所有证据都只到"轨迹不一样"，现在至少排除了"锚定会让产出变差"这一种可能。

**不能说的**：不能据此说锚定没用。**两条臂都撞到了天花板**——3/3 对 3/3，一个双方都必过的任务本来就没有分辨力。n=3 的零结果排除的是大效应，不是小效应。

### 任务还是不够难

这是本轮最实在的收获。L1 已经是三个跨包回归、15 个失败测试，两条臂仍然全过，步数 22–51。而观测语料里两种轨迹显出差别的区间是 144–500 步。

`headless` 一次性驱动本身就比交互式紧凑得多。想进到那个区间，靠"再多种几个 bug"大概不行——修 bug 这类任务有明确的成功判据和收敛路径，模型不会在里面绕。要拉开差距，任务得是**探索空间大、正确解不唯一、需要反复推翻重来**的那种，而那种任务恰恰难以自动判分。

这是产出级测量的真正矛盾：**能自动判分的任务，往往简单到两种条件都能过；能分辨条件的任务，往往判分要靠人。** 下一步如果继续，得先解决这个矛盾，而不是继续加难度。
