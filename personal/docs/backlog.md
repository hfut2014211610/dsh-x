...
| # | 事项 | 已做的 | 还差的 |
|---|---|---|---|
| B | 加「关于」菜单，呈现更新日志 + 版本更新操作 | 托盘有「关于」（版本、运行时来源、窗口 origin）；更新日志已进「有新版本」对话框（`65e0446907`，取自 release 自己的 notes，截断到 12 行） | **web 端入口没有** —— 浏览器里用的人看不到托盘。要做得往 `settings.section` 挂一个 fork 自有包，跟连接器页同一条路 |
