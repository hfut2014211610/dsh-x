...
The feature extends the documented Session-event, subagent, Typert remote, and client slot mechanisms rather than changing `agent-loop`. Session events are the durable facts, the Web Client owns the game interaction surface, and fresh `spawn` children support persona, tool filtering, depth limits, and structured output; see [the architecture map](../../../../docs/architecture.md), [the subagent contract](../../../../docs/subsystems/subagent.md), and [the Web Client architecture](../../../../packages/client/README.md).
...
This mode is UI-only. It does not register slash commands, reuse the Chat composer, parse natural-language instructions, or provide a command fallback. The game surface is the sole supported start, action, resume, and abort-game surface. While a Werewolf Host is selected, ordinary app chrome is unavailable until the player uses the dedicated exit.

### Dedicated game surface

The Web Client injects a `shell.surface` chain entry that elects Host Sessions whose confirmed `agentPreset` is `werewolf`. An ordinary `werewolf` Session renders the game lobby; `start` creates the dedicated Host and navigates to it. Once elected, the entry replaces the standard preset switcher, Session sidebar, conversation, details, overlays, and resize handles as one unit. Its dedicated exit clears the selected Session without aborting the game, so reopening the Host returns directly to its active or paused table. Only a bound game Host accepts mutation methods.
...
| `client/ui-werewolf/` (under `packages/`, stage 4) | `shell.surface` registration, Typert client binding, dedicated table, lobby, role reveal, action forms, timeline, result/replay, responsive layout, accessibility, and localized copy |
...
Stages 1–4 have landed ([deterministic core](../../implemented/feature/2026-08-21-werewolf-deterministic-core.md), [bot runner](../../implemented/feature/2026-08-21-werewolf-bot-runner.md), [Session Host](../../implemented/feature/2026-08-21-werewolf-session-host.md), [game surface](../../implemented/feature/2026-08-21-werewolf-conversation-view.md)); stage 5 remains proposed.
...
4. Add the dedicated `werewolf` game surface, lobby, covered role reveal, table, generic human action form, spectator state, result/replay, responsive and accessibility tests, and the optional bundle composition. Do not add a slash command or Chat-composer route.
...
- Lobby, role reveal, night action, day discussion, vote, spectator, pause/resume, result, review, and new-game flows work entirely inside the dedicated `werewolf` surface. While selected, no ordinary preset switcher, Session navigation, conversation, details, or generic overlay is mounted; the dedicated exit restores the standard shell without aborting the game. The plugin registers no slash command, and Chat text cannot mutate game state. RPC tests prove that the runtime derives the participant from the platform principal and rejects caller-selected Host Sessions or seats.
