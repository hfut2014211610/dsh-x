...
The bundled runtime ships as one archive and the shell extracts it into userData on first run, checksum-gated per archive (`src/bundled-runtime.ts`) — this electron-builder build strips `node_modules` from resource copies outright, so an installed tree cannot ride `extraResources` directly. The extracted runtime runs under the Electron binary through `ELECTRON_RUN_AS_NODE` with `--expose-internals` (the web profile's HMR row requires it), so an installed app needs no system Node.js — the PATH and npx sources exist for development machines that already have one. Windows shell spawns quote every argv element: the app installs under `DeepSeek Harness.exe`, and an unquoted path splits mid-path in cmd. The Electron binary download is opt-in (`allowBuilds: electron: false` plus `pnpm run desktop:prepare`), so every other CI lane's `pnpm install` stays as fast as before.
