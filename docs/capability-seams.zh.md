...
flowchart LR
  ...
  svc_documents["ctx.documents<br/>Versioned document access for writing mode"]
  ...
  pkg_game["game"]
  svc_games["ctx.games<br/>Session-backed game Host"]
  pkg_werewolf["werewolf"]
  svc_werewolf["ctx.werewolf<br/>Werewolf definition registries"]
  pkg_werewolf_classic["werewolf-classic"]
  svc_werewolfGame["ctx.werewolfGame<br/>Typed Werewolf Host gateway"]
  ...
  pkg_documents --> svc_documents
  pkg_documents_local --> svc_documents
  pkg_e2b --> svc_e2b
  ...
  pkg_werewolf --> svc_werewolf
  pkg_werewolf --> svc_werewolfGame
  pkg_werewolf_classic --> svc_werewolf
  ...
  svc_documents --> pkg_tool_documents
  svc_dynamicCordisRunner --> pkg_tool_cordis
  svc_e2b --> pkg_fs_e2b
  svc_e2b --> pkg_subprocess_e2b
  svc_fs --> pkg_tool_fs
  svc_games --> pkg_werewolf
...
| `ctx.documents` | `seam` | [`documents`](../packages/writing/documents) | [`documents-local`](../packages/writing/documents-local) | [`tool-documents`](../packages/writing/tool-documents) | - | list/read/outline/search/create 加带版本守卫的 apply；提供方发出 documents/changed，浏览器 UI 经生成的 remote 跟随已提交的修改。 |
...
| `ctx.games` | `seam` | [`game`](../packages/game/game) | - | [`werewolf`](../packages/game/werewolf) | - | 该包同时提供游戏模块约定和默认 Session provider：每局使用一个空闲 Host Agent，原子提交回执与领域事件，并负责主体绑定、变更串行化、投影失效和有界的 fresh 子代理调度。 |
| `ctx.werewolf` | `seam` | [`werewolf`](../packages/game/werewolf) | [`werewolf-classic`](../packages/game/werewolf-classic) | - | - | 面向规则集、角色、阶段与胜利条件的可信同进程注册表；经典定义注册其上，规则编译按准确版本解析。 |
| `ctx.werewolfGame` | `core` | [`werewolf`](../packages/game/werewolf) | - | - | - | 注册轻量狼人杀游戏模块，并向专用 Web UI 暴露已解析主体的开始、视图、行动、恢复、中止和终局回放方法。 |
