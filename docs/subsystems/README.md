...
| [writing.md](writing.md) | writing mode: the document capability vocabulary, `ctx.documents`, `documents/changed`, and the `writing` preset surface |
...
| [werewolf.md](werewolf.md) | the deterministic Werewolf game core: rule-set compilation over trusted registries, the `werewolf/*` event family, the pure phase engine, and bot continuity context |
