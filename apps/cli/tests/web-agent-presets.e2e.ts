...
/**
 * The shell tool a `standard`-family preset actually carries here.
 *
 * The shipped composition mounts `tool-bash` on POSIX and `tool-pwsh` on
 * Windows, so a catalog asserted as `bash` is asserting the host, not the
 * product. `minimal` and `anchored-standard` are not in this family: their
 * `custom-bash` row registers a Git Bash tool under the `bash` name on
 * win32, which is why those catalogs stay literal.
 */
...
describe('the shipped Web composition', () => {
  ...
  it('supplies every shipped preset, and only those, from the system root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual([
      'anchored-standard', 'cordis', 'minimal', 'ptc', 'standard', 'ued', 'writing',
    ])
  ...
  it('composes the exact RL prompt and persistent shell from `minimal`', async () => {
    ...
    try {
      ...
      const executed = await ctx.tools.execute({
        callId: ToolCallId('preset-minimal-bash'),
        name: 'bash',
  ...
  it('anchors `anchored-standard` on the Minimal pair, then promotes to a resident, unlockable set', async () => {
    ...
    try {
      ...
      const booted = await ctx.tools.execute({
        callId: ToolCallId('preset-anchored-bash'),
        name: 'bash',
        arguments: { command: 'printf dsh-bash-ok' },
        signal: new AbortController().signal,
        agent: handle.agent,
      ...
      session.append('assistant/message', {
        ...
        message: createAssistantMessage({
          ...
          source: { provider: 'spec', model: 'spec' },
        }),
        stream: [],
      }, { surfaceOp: 'append' })
      ...
      const searched = await ctx.tools.execute({
        callId: ToolCallId('preset-anchored-search'),
      ...
      const unlockCall = ToolCallId('preset-anchored-unlock')
