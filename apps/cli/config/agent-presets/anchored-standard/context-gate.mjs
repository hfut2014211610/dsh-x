/**
 * anchored-context-gate — unified injection control for this preset.
 *
 * Ported from the community preset `dsh-anchored-standard` (MIT).
 *
 * While a session is unpromoted, this one plugin keeps its first model
 * request free of auto-injected context, whatever the source, by intercepting
 * the harness's two unified injection paths — not a per-source denylist — so
 * it covers sources that do not exist yet:
 *
 ...
 *     pre-step waterfall.
 *
 * The phase is the same epoch-aware promotion machine the preset's other
 * rows use (see compaction-epoch.mjs): a durable `tool/call` and/or
 * `assistant/message` (per `promoteOn`, default `either`) promotes, and a
 * `compaction/end` boundary demotes again — the first post-compaction
 * request is a "second first request" and is gated the same way. Derived
 ...
 * first request. Keep this flag in sync with the companion tool-bootstrap
 * row.
 *
 * CONFIG:
 ...
 * transform — nothing registered later re-injects past it (measured by the
 * upstream reproduction: with the skill catalog present, 0/9 sessions
 * anchored).
 *
